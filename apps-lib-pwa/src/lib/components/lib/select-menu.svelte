<script lang="ts">
    import { fmt_cl, type ISelect, parse_layer } from "@radroots/apps-lib";
    ...
    const classes_layer = $derived(
        typeof basis?.layer === `boolean` ? `` : `text-ly${layer}-gl`,
    );
</script>
...
>
    <div
        class={`${fmt_cl(
            basis.classes,
        )} z-20 absolute top-0 left-0 flex flex-row h-full w-full justify-end items-center ${classes_layer}`}
    >
        <select
