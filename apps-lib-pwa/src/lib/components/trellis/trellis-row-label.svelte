<script lang="ts">
	...
	import GlyphButton from "../buttons/glyph-button.svelte";
			...
			{#each basis.right.reverse() as title_r}
				...
				>
					...
					{:else if `value` in title_r}
						<p
							class={`${fmt_cl(title_r.classes)} font-sans text-line_d text-ly${layer}-gl_d ${title_r.hide_truncate ? `` : `truncate`} el-re`}
						>
							{title_r.value || ``}
						</p>
					{/if}
