...
>
    <div
        class={`relative flex flex-col h-auto w-lo_${$app_lo} gap-[3px] ${set_title_background ? `bg-ly${basis.layer}` : ``}`}
    >
        ...
        {:else if basis.list}
            <div class={`flex flex-col w-full justify-center items-center`}>
                {#each basis.list as li}
                    {#if li}
                        <div
                            class={`${li.hide_field ? `hidden` : ``} group flex flex-row h-full w-full justify-end items-center bg-ly${basis.layer}  ${li.full_rounded ? `rounded-touch` : ``} ${hide_rounded ? `` : `first:rounded-t-2xl last:rounded-b-2xl`} ${!li.hide_active ? `active:bg-ly${basis.layer}_a` : ``} el-re`}
                        >
                            <div
    ...
    {#if el_append}
        <div
            class={`flex flex-col w-full ${set_default_background ? `bg-ly${basis.layer}` : ``}`}
        >
            {@render el_append()}
        </div>
    {/if}
