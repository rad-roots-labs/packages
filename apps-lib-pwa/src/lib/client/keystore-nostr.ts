import { keystore } from "@radroots/client";
import { err_msg, handle_err, type ResolveError, type ResultObj, type ResultPass, type ResultPublicKey, type ResultSecretKey, type ResultsList } from '@radroots/utils';
import { lib_nostr_key_generate, lib_nostr_public_key, lib_nostr_secret_key_validate } from '@radroots/utils-nostr';

export type INostrKeystore = {
...
export class PwaKeystoreNostr implements INostrKeystore {
    private static readonly keystore_database = "radroots-pwa-v1-keystore";
    private static readonly keystore_store = "nostr";

    private _keystore: keystore.WebKeystore;

    constructor() {
        this._keystore = new keystore.WebKeystore({
            database: PwaKeystoreNostr.keystore_database,
            store: PwaKeystoreNostr.keystore_store,
        });
    }
